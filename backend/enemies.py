...
class RunnerEnemy(Enemy):
    ...
    def __init__(self, enemy_id: int, wave_number: int):
        super().__init__(enemy_id, wave_number)
        self.enemy_type = "runner"
        self.hp = 30 + (wave_number * 8)
        self.max_hp = self.hp
...
class HealerEnemy(Enemy):
    ...
    def __init__(self, enemy_id: int, wave_number: int):
        super().__init__(enemy_id, wave_number)
        self.enemy_type = "healer"
        self.hp = 60 + (wave_number * 12)
        self.max_hp = self.hp
...
def get_wave_composition(wave_number: int) -> List[str]:
